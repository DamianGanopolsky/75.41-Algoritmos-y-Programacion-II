...
size_t hash_str( const char *clave,size_t capacidad) {
...
void avanzar_en_hash(hash_iter_t* iter){
    ...
    if (hash_cantidad(hash) > 0){
        ...
        while (hash->listas[iter->pos] == NULL && iter->pos < hash->capacidad - 1)
...
void completar_nuevo_hash_y_borrar_anterior(hash_t* hash,lista_t** listaz,size_t nuevo_tam){
...
void redimensionar(hash_t* hash,size_t nuevo_tam){
    ...
    completar_nuevo_hash_y_borrar_anterior(hash,listaz,nuevo_tam);
...
hash_iter_t *hash_iter_crear(const hash_t *hash){
    ...
    if (iter->hash->cantidad > 0){
        ...
        if (iter->hash->listas[iter->pos] != NULL){
...
bool hash_iter_avanzar(hash_iter_t *iter){
    ...
    if (lista_iter_al_final(iter->iter_lista)){
        ...
        if (iter->pos < hash->capacidad - 1) iter->pos += 1;

        avanzar_en_hash(iter);

        if (iter->pos < hash->capacidad && hash->listas[iter->pos] != NULL){ 
            iter->iter_lista = lista_iter_crear(hash->listas[iter->pos]);
            if(iter->iter_lista==NULL) return false;
            return true; 
        }
